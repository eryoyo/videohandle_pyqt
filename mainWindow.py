...
class Ui_MainWindow(QObject):
    ...
    def handleVideo(self, cur):
        ...
        if cap.isOpened():
            ...
            while True:
                ...
                if not ret:
                    break
                self.handleEachFrame(img_src, cur['index'])
                i += 0
                self.num_handled += 1
        else:
            print('视频打开失败！')
    ...
    '''
    向后端发送请求，传送图片得到处理的结果，将数据塞到消息队列
    @param:
        img: opencv读取的图片
        index: 当前处理的视频文件的编号，因为最后后端处理完成之后结果都会塞到一个消息队列之中所以需要分辨不同的视频文件处理
    '''

    def handleEachFrame(self, img, index):
        print(index)
